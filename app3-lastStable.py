...
class f_string:
    def __init__(self,string:str|Callable,map:dict[str,Any]):
        self.string = string
        self.map = map
    ...
    def using(self,*args):
        string = self.string
        if isinstance(string,Callable): string = string()
        for i in self.map:
            insert = self.map[i]
            if callable(insert):
...
def getRandomMorning() -> str:
    match randint(1,5):
        case 1: return "$name would you like to get up?"
        case 2: return "Wake wake $name, time to get up"
        case 3: return "Its about time to get up now $name."
        case 4: return "I'll make coffee if you get up right now $name."
        case 5: return "Its time $name."
    return "$name please get up, something went wrong..."

story = {
    'enter': branch(
        "You wake up, your eyes still closed.",
        {'':'request name'}
    ),
    'request name': branch(
        "Good morning... (Say your name)",
        {},{
            'after': lambda res, worked: 
                (print(res),setName(res), reroute("morning"))
        }
    ),
    "morning": branch(
        f_string(lambda: getRandomMorning(), {
            'name': lambda _: name
        }),
        {"yes":"get up","no":"staybed"}
    ),
    
    "staybed": branch(
        "I see, sleep for another 5, then I'll ask again",
        {'':'sleep0'},
    ),
    
    'sleep0': branch('',{'':'sleep1'}),
    'sleep1': branch('',{'':'sleep2'}),
    'sleep2': branch('',{'':'sleep3'}),
    'sleep3': branch('',{'':'morning'}),
    
    'getup': branch(
        'You get out of bed slowly. By now light is pouring into your room. Such calm times are sure to continue.',
        {'':'routine0'}
    ),
    'routine0': branch(
        'What would you like to do?',{
            'brush':'morning_brush',
            'get dressed':'morning_dress_up',
            'go eat': ''
        }
    ),
